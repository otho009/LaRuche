...
Après avoir désigné les fonctionnalités que nous prévoyions d'implémenter à notre produit, nous avons jugé nécessaire de pouvoir recevoir un retour sur cette concéption initiale. \\ 
Ainsi, une expression détaillée des besoins d'un utilisateur potentiel nous semblait très utile dans cette phase de planification. Car, effectivement, une expression validée des besoins donnée en entrée de réalisation nous permettrait de vérifier que notre  concéption  répond bel et bien au besoin des utilisateurs.\\
De plus, cela nous servirait à éliminer certaines fonctionnalités, prévues initialement et qui peuvent se révéler finalement inutiles à l'usage, ainsi que d'inclure d'autres qui peuvent donner plus de valeur au produit. 
...
Lors de cet entretien, nous avons commencé tout d'abord par présenter l'ensemble des fonctionnalités prévues, pour ensuite don	ner la parole à notre invité qui dans le cadre de notre modélisation, représente un potentiel utilisateur de type « Fournisseur ». \\
À l'issue de cet entretien, nous avons eu un aperçu de ce que peut exiger un fournisseur de ce projet. 
En prenant en compte ces exigences, nous avons pu modifier notre  concéption  initiale, notamment en redéfinissant les notions de « ruches » et de « cellules ». Évitant ainsi, un futur déphasage entre le besoin initial et l'application réalisée.




 Étant donnée, l'apport considérable qu'a pu avoir cette expérience, nous prévoyons de reproduire des expériences similaires. Afin d'impliquer les utilisateurs du début à la fin du projet. Maintenant ainsi le paradigme Agile que nous avons adopté pour gérer ce projet.
